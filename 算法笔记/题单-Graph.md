---
title: 图论算法题单
aliases: [图论算法（DFS/BFS/拓扑排序/基环树/最短路/最小生成树/网络流）, 灵神图论算法题单]
tags:
  - leetcode
  - graph
  - algorithm
...
original: https://leetcode.cn/circle/discuss/01LUak
last_update: 2025-10-13 02:48:30
---

# 图论算法（DFS/BFS/拓扑排序/基环树/最短路/最小生成树/网络流）

来源：https://leetcode.cn/circle/discuss/01LUak
更新：2025-10-13 02:48:30
...
> int x = q.poll();
> for (int y : g[x]) {
> if (dis[y]  bfs(int n, vector>& edges, int start) {
> vector> g(n);
> for (auto& e : edges) {
> int x = e[0],...
...
> 可以锻炼状态设计能力。
> **专题：跳跃游戏**
> 注：关于**网格图**的 DFS 和 BFS，请看 [网格图题单](/lc-rating/v0/list/grid)。
...
> q.append(y)  # 加入学习队列
> if len(topo_order)  topologicalSort(int n, int[][] edges) {
...
- [ ] [3481. 应用替换](https://leetcode.cn/problems/apply-substitutions/)（会员题）
...
- [ ] [1361. 验证二叉树](https://leetcode.cn/problems/validate-binary-tree-nodes/) 1465
...
- [ ] [1136. 并行课程](https://leetcode.cn/problems/parallel-courses/) 1710（会员题）
...
> for y, wt in g[x]:
> new_dis_y = dis_x + wt
> if new_dis_y [] g = new ArrayList[n]; // 邻接表
> Arrays.setAll(g, _ -> new ArrayList<>());
> for (int[] e : edges) {
> int x = e[0];
> int y = e[1];
...
- [ ] [3342. 到达最后一个房间的最少时间 II](https://leetcode.cn/problems/find-minimum-time-to-reach-last-room-ii/) 1862
...
- [ ] [3599. 划分数组得到最小 XOR](https://leetcode.cn/problems/partition-array-to-minimize-xor/) 1955
...
> Kruskal 算法模板（用到了并查集，完整模板见 [数据结构题单](/lc-rating/v0/list/data_structure)）：
> ```py [sol-Python3]
> class UnionFind:
...
- [ ] [1135. 最低成本连通所有城市](https://leetcode.cn/problems/connecting-cities-with-minimum-cost/) 1753（会员题）
- [ ] [1168. 水资源分配优化](https://leetcode.cn/problems/optimize-water-distribution-in-a-village/) 2069（会员题）
- [ ] [3219. 切蛋糕的最小总开销 II](https://leetcode.cn/problems/minimum-cost-for-cutting-cake-ii/) 1789
...
- [ ] [3385. 破解锁的最少时间 II](https://leetcode.cn/problems/minimum-time-to-break-locks-ii/)（会员题）
- [ ] [1066. 校园自行车分配 II](https://leetcode.cn/problems/campus-bikes-ii/) 1885（会员题）
...
- [ ] [2493. 将节点分成尽可能多的组](https://leetcode.cn/problems/divide-nodes-into-the-maximum-number-of-groups/) 2415
...
- [ ] [3656. 判断是否存在简单图](https://leetcode.cn/problems/determine-if-a-simple-graph-exists/)（会员题）

### 十、树上算法

> 见 [链表、树、回溯](/lc-rating/v0/list/trees) 题单的第三章节。
